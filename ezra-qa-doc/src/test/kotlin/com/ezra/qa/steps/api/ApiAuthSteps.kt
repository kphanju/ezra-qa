...
class ApiAuthSteps(
    ...
    init {
        Given("User A is authenticated with valid credentials") {
            ...
            Assertions.assertThat(response.code).`as`("User A auth").isEqualTo(200)
            val auth = client.parseAuth(body)
            Assertions.assertThat(auth.access_token).isNotBlank()
        ...
        Given("User B is authenticated with valid credentials") {
            ...
            context.capture(response, body)
            Assertions.assertThat(response.code).`as`("User B auth").isEqualTo(200)
            val auth = client.parseAuth(body)
    ...
    private fun extractSub(jwt: String): String = try {
        ...
        val decoded = String(Base64.getUrlDecoder().decode(pad(payload)))
